...
(кроме .NET 9 или новее для запуска самого адаптера).
...
Пакет называется `dotnet-debugger-dap`, команда — `dotnet-debugger`. Пакет один на все платформы.
Расширение VS Code без встроенного адаптера тоже найдёт установленный так инструмент.

### Архив

В релизах есть самостоятельные сборки адаптера по платформам (`dotnet-debugger-<версия>-<rid>.zip|tar.gz`).
